...
gulp.task('sass-sources', function () {
    return gulp.src('./scss/*.scss').pipe(sourcemaps.init()).pipe(sass().on('error', sass.logError)).pipe(sourcemaps.write('./maps')).pipe(gulp.dest('./css'));
});
...
gulp.task('sass', function (event) {
    ...
    }).on('error', sass.logError)).pipe(gulp.dest(destinations.css));
});
...
gulp.task("babel", function () {
    return gulp.src("./*.js").pipe(babel()).pipe(gulp.dest("./js"));
});
// Server
